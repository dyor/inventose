...
namespace INV.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginRegisterViewModel model, string returnUrl = null)
        {
            ...
            return View("Login", new LoginRegisterViewModel() { LoginViewModel = model.LoginViewModel });
        }
        ...
        public async Task<IActionResult> Register(LoginRegisterViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                //first need to see if username exists
                var possibleUserName = model.RegisterViewModel.Email.Remove(model.RegisterViewModel.Email.IndexOf('@'), model.RegisterViewModel.Email.Count() - model.RegisterViewModel.Email.IndexOf('@'));
                var existingUser = await _userManager.FindByNameAsync(possibleUserName); 
                if (existingUser != null)
                {
                    possibleUserName = model.RegisterViewModel.Email;
                }
                var user = new ApplicationUser { UserName = possibleUserName, Email = model.RegisterViewModel.Email };
            ...
            // If we got this far, something failed, redisplay form
            return View("Login", new LoginRegisterViewModel() { RegisterViewModel = model.RegisterViewModel });
        ...
        public async Task<IActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                await _emailSender.SendEmailAsync(model.Email, "Reset Password",
                   $"Please reset your password by clicking visiting: {callbackUrl}");
                return View("ForgotPasswordConfirmation");
            }
